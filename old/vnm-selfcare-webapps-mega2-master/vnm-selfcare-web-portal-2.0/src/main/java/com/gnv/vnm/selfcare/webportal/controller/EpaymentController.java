 ...
 * @author nandipinto
 *
 * Previously ETopupController
 */

@Controller
@RequestMapping(value = "/secure/epay")
public class EpaymentController extends WebPortalBaseController {
	
	private static final Logger logger = LoggerFactory.getLogger(EpaymentController.class);
	
	public static final String REFERER_PETIE = "PETIE"; 
	
	static final String IMG_STRING = "_r/images/";
	
	@GetMapping(value = "/init/{msisdn}/{amount}/{trxType}")
	public ResponseEntity<?> init(@PathVariable String msisdn, @PathVariable double amount,
			@PathVariable int trxType, HttpServletRequest request){
		...
		try {
			transactionType = TransactionType.valueOf(trxType);
		} catch (Exception e) {}
		
		try {
			orderId = paymentGatewayService.initTransaction(msisdn, itemCode, amount, ClientAppId.WEB_APP,
					transactionType, null, referrer, request.getSession().getId());
		} catch (Exception e) {
