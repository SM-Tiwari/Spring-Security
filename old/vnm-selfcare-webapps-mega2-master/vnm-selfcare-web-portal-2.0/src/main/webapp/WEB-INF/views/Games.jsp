	...
	<div class="marketPage">
			...
			<div id="gameBanner" class="carousel slide" data-ride="carousel" data-interval="3000">
				...
				<div class="carousel-inner">
					...
					<div class="item <c:if test="${ctr.index eq 0}">active</c:if>">
						<img src="${banner.webBanner}" alt="Game Banner ${ctr.index}" style="width: 100%;"/>
					</div>
					</c:forEach>
				</div>
		...
		<div class="container">
			<div class="col-sm-10 col-sm-offset-1">
				<c:forEach items="${gameCategories}" var="category">
					...
					<c:if test="${category.itemCount gt 0}">
						...
						<div class="row marketPage-sectionTitle">
							...
							<div class="col-xs-6 sectionTitle-more">
								...
								<a href="${pageContext.request.contextPath}/shop/content/games/more/${category.id}/0"><spring:message code="label.More" text="More"/> <i class="material-icons">navigate_next</i></a>
								</c:if>
							</div>
						</div>
						...
						<div class="row marketPage-sectionGame">
							<c:forEach items="${requestScope[category.code]}" var="game" varStatus="ctr">
								<c:if test="${ctr.index lt 4}">
								<div class="col-xs-6 col-sm-3">
						            <div class="thumbnail">
						                <div class="card-wrapper">
						                    <a href="${pageContext.request.contextPath}/shop/content/games/details/${category.id}/${game.id}"><img src="${!empty game.defaultIcon ? game.defaultIcon : game.defaultImage}" alt="${utils:abbreviate(game.titleEN, 35)}"></a>
						                </div>
						                <div class="caption text-center gameName">
				                            <a href="${pageContext.request.contextPath}/shop/content/games/details/${category.id}/${game.id}">
				                            <c:choose>
				                              <c:when test="${pageContext.response.locale.language eq 'en'}"><h6>${utils:abbreviate(game.titleEN, 35)}</h6></c:when>
				                              <c:otherwise><h6>${utils:abbreviate(game.titleVI, 35)}</h6></c:otherwise>
				                            </c:choose>
				                            </a>
										</div>
						            </div>
