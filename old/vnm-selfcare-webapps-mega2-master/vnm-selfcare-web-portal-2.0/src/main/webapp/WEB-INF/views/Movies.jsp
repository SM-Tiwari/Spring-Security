...
<head>
	...
	<title>My Vietnamobile - Movies</title>
...
<div class="wrapper">
	...
	<div class="marketPage">
			...
			<div class="col-xs-12 marketPage-banner">
				<!-- <img src="${pageContext.request.contextPath}/static/assets/image/banner.jpg"> -->
				<div id="movieBanner" class="carousel slide" data-ride="carousel" data-interval="3000">
					<!-- Indicators -->
					<ol class="carousel-indicators">
						<c:forEach items="${movieBanner}" var="banner" varStatus="ctr">
							<li data-target="#movieBanner" data-slide-to="${ctr.index}" <c:if test="${ctr.index eq 0}">class="active"</c:if>></li>
						</c:forEach>
					</ol>
					...
					<div class="carousel-inner">
						...
						<div class="item <c:if test="${ctr.index eq 0}">active</c:if>">
							<img src="${banner.webBanner}" alt="Movie Banner ${ctr.index}" class="img-responsive" style="width: 100%;"/>
						</div>
						</c:forEach>
					</div>

					<!-- Left and right controls -->
					<a class="left carousel-control" href="#movieBanner"
						data-slide="prev"> <span
						class="glyphicon glyphicon-chevron-left"></span> <span
						class="sr-only">Previous</span>
					</a> <a class="right carousel-control" href="#movieBanner"
			...
			<div class="container">
				<div class="col-sm-10 col-sm-offset-1">
					<c:forEach items="${movieCategories}" var="category">
						<c:if test="${category.itemCount gt 0}">
							...
							<div class="row marketPage-sectionTitle">
								...
								<div class="col-xs-6 sectionTitle-more">
									...
									<a href="${pageContext.request.contextPath}/shop/content/movie/more/${category.id}/0"><spring:message code="label.More" text="More"/> <i class="material-icons">navigate_next</i></a>
									</c:if>
								</div>
							</div>
							...
							<div class="row marketPage-sectionMovie">
								<c:forEach items="${requestScope[category.code]}" var="movie" varStatus="ctr">
									<c:if test="${ctr.index lt 4}">
									<div class="col-xs-6 col-sm-3">
							            <div class="thumbnail">
							                <div class="card-wrapper">
							                    <a href="${pageContext.request.contextPath}/shop/content/movie/details/${category.id}/${movie.id}"><img src="${!empty movie.defaultIcon ? movie.defaultIcon : movie.defaultImage}" alt="${utils:abbreviate(movie.titleEN, 35)}"></a>
							                </div>
							                <div class="caption text-center movieName">
				                              <a href="${pageContext.request.contextPath}/shop/content/movie/details/${category.id}/${movie.id}">
				                                <c:choose>
				                                  <c:when test="${pageContext.response.locale.language eq 'en'}"><h6>${utils:abbreviate(movie.titleEN, 35)}</h6></c:when>
				                                  <c:otherwise><h6>${utils:abbreviate(movie.titleVI, 35)}</h6></c:otherwise>
				                                </c:choose>
				                              </a>
				                            </div>
							            </div>
