...
<head>
	...
	<title>My Vietnamobile - Notifications</title>
		...
		<div class="container">
			<div class="col-sm-11 col-sm-offset-1">
				<ul>
					...
					<li><spring:message code="label.TopMenu.Notifications" text="Notifications"/></li>
				</ul>
				<h1><spring:message code="label.TopMenu.Notifications" text="Notifications"/></h1>
			</div>
		</div>
	</div>
		...
		<div class="col-sm-8 col-sm-offset-2">
			<div class="card notifCard">
				<div class="notif-title">
					<h6><spring:message code="label.TopMenu.Notifications" text="Notifications"/></h6>
				</div>
				<div class="notif-body">
					<c:forEach items="${notificationList}" var="pushedNotif" varStatus="ctr">
						<c:choose>
							<c:when test="${pageContext.response.locale.language eq 'en'}"><h6>${pushedNotif.titleEN}</h6><p>${pushedNotif.detailsEN}</p></c:when>
							<c:otherwise><h6>${pushedNotif.titleVI}</h6><p>${pushedNotif.detailsVI}</p></c:otherwise>
						</c:choose>					
						<c:if test="${ctr.index < messageCount - 1}"><div class="divider"></div></c:if>	
					</c:forEach>
				</div>
			</div>
		</div>
