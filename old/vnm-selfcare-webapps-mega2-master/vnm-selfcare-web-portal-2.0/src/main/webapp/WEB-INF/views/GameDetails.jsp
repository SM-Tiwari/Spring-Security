...
<head>
	...
	<title>My Vietnamobile - Game Details</title>
...
<div class="wrapper">
	<div class="marketPage">
		...
		<div class="container-fluid">
			<div class="col-sm-11 col-sm-offset-1" style="padding: 0;">
				<ul class="mod-breadcrumb">
					...
					<li><a href="${pageContext.request.contextPath}/shop/content/games/more/${categoryId}/0"><spring:message code="label.TopMenu.Games" text="Games"/></a></li>
				</ul>
			</div>
		</div>
		...
		<div class="col-xs-12 marketPage-banner">
			<img src="${gameDetails.defaultBanner}">
		</div>
			...
			<div class="col-sm-10 col-sm-offset-1">
				...
				<div class="row gameDetails">
					<div class="col-xs-6 col-sm-3">
						<div class="gameDetails-image">
							<img src="${!empty gameDetails.defaultIcon ? gameDetails.defaultIcon : gameDetails.defaultImage}">
						</div>
					</div>
					<div class="col-xs-6 col-sm-4">
						<div class="gameDetails-title">
						    <c:choose>
						      <c:when test="${pageContext.response.locale.language eq 'en'}"><h1>${gameDetails.titleEN}</h1></c:when>
						      <c:otherwise><h1>${gameDetails.titleVI}</h1></c:otherwise>
						    </c:choose>	
							<p>${gameDetails.producers}</p>
						</div>
					...
					<div class="col-xs-12 col-sm-5">
						<!-- ${gameDetails.iosProductURL}, ${gameDetails.androidProductURL} -->
						<div class="buyGameCTA">
							<button class="btn btn-primary btn-round" id="btnDownload" onclick="downloadContent('${gameDetails.iosProductURL}', '${gameDetails.androidProductURL}');">
								<span style="font-weight: 600; font-size: 18px;"><spring:message code="label.Play" text="Play"/></span>
								<!-- <p><fmt:formatNumber value="${gameDetails.price}" pattern="#,##"/>₫</p>-->
							</button>
							<table class="table table-condensed movieMeta">
								<tr>
									...
									<td class="col-xs-6">
									    <c:choose>
									      <c:when test="${pageContext.response.locale.language eq 'en'}">${gameDetails.categoryNameEN}</c:when>
									      <c:otherwise>${gameDetails.categoryNameVI}</c:otherwise>
									    </c:choose>										
									</td>
								</tr>
								<tr>
									<th><spring:message code="label.Games.ReleaseDate" text="Release Date"/></th>
									<td><fmt:formatDate value="${gameDetails.releaseDate}" pattern="dd-MM-yyyy"/></td>
								</tr>
								<tr>
									<th><spring:message code="label.Games.Version" text="Version"/></th>
									<td>${gameDetails.version}</td>
								</tr>
								<tr>
									<th><spring:message code="label.Games.Updated" text="Updated"/></th>
									<td><fmt:formatDate value="${gameDetails.lastUpdatedDate}" pattern="dd-MM-yyyy"/></td>
								</tr>
								<tr>
									<th><spring:message code="label.Games.Size" text="Size"/></th>
									<td><c:if test="${!empty gameDetails.fileSize and gameDetails.fileSize gt 0 }"><fmt:formatNumber value="${gameDetails.fileSize / 1024}" maxFractionDigits="2"/> MB</c:if></td>
								</tr>
							</table>
						</div>
					</div>
				...
				<div class="row marketPage-sectionTitle">
					...
					<div class="col-xs-12 sectionTitle-storyline">
					    <c:choose>
					      <c:when test="${pageContext.response.locale.language eq 'en'}"><p>${gameDetails.descriptionEN}</p></c:when>
					      <c:otherwise><p>${gameDetails.descriptionVI}</p></c:otherwise>
					    </c:choose>										
					</div>
				</div>

				<!-- Screenshot : Title -->
				<c:if test="${fn:length(gameDetails.screenShots) gt 0}">
				<div class="row marketPage-sectionTitle">
				...
				<div class="row marketPage-sectionGame">
					<c:forEach items="${gameDetails.screenShots}" var="screenshot" varStatus="ctr">
				...
				<div class="row marketPage-sectionTitle">
					<c:choose>
						<c:when test="${!empty relatedGames}">
							<div class="col-xs-6 sectionTitle-title">
								<h2><spring:message code="label.Games.RelatedGames" text="Related Games"/></h2>
							</div>
							<div class="col-xs-6 sectionTitle-more">
								<c:if test="${fn:length(relatedGames) gt 4}">
									<a href="${pageContext.request.contextPath}/shop/content/games/more/${categoryId}/0">
				...
				<div class="row marketPage-sectionGame">
					<c:forEach items="${relatedGames}" var="relatedGame" varStatus="ctr">
						<c:if test="${ctr.index lt 4}">
						<div class="col-xs-6 col-sm-3">
				            <div class="thumbnail">
				                <div class="card-wrapper">
				                    <a href="${pageContext.request.contextPath}/shop/content/games/details/${categoryId}/${relatedGame.id}"><img src="${!empty relatedGame.defaultIcon ? relatedGame.defaultIcon : relatedGame.defaultImage}" alt="${relatedGame.titleEN}"></a>
				                </div>
				                <div class="caption text-center gameName">
				                <a href="${pageContext.request.contextPath}/shop/content/games/details/${categoryId}/${relatedGame.id}">
							      <c:choose>
							        <c:when test="${pageContext.response.locale.language eq 'en'}"><h6>${utils:abbreviate(relatedGame.titleEN, 25)}</h6></c:when>
							        <c:otherwise><h6>${utils:abbreviate(relatedGame.titleVI, 25)}</h6></c:otherwise>
							      </c:choose>										
				                </a>
				                </div>
				            </div>
...
$(document).ready(function() {
	...
	var flag = true;
	if (os == "iOS" || os == "Mac OS"){
		if ("${gameDetails.iosProductURL}" != ""){
			flag = false;
		}
	} else {
		if ("${gameDetails.androidProductURL}" != ""){
			flag = false;
		}
	}
