				...
				<div class="col-sm-10 col-sm-offset-1">
                   	<div class="row marketPage-sectionTitle">
						<div class="col-xs-6 sectionTitle-prev">
							...
							<a href="${pageContext.request.contextPath}/shop/content/movie/more/${categoryId}/${paging.number - 1}"><i class="fa fa-angle-left" aria-hidden="true"></i> <spring:message code="label.Back" text="Back"/></a>
							<%}%>
						</div>
						<div class="col-xs-6 sectionTitle-next">
							<%if (paging.hasNext()){%>
							<a href="${pageContext.request.contextPath}/shop/content/movie/more/${categoryId}/${paging.number + 1}"><spring:message code="label.Next" text="Next"/> <i class="fa fa-angle-right" aria-hidden="true"></i></a>
							<%}%>
						</div>
					</div>
					<!-- Section Movie List -->
					<div class="row marketPage-sectionMovie">
						<c:forEach items="${movieList}" var="movie" varStatus="ctr">
							<div class="col-xs-6 col-sm-3">
					            <div class="thumbnail">
					                <div class="card-wrapper">
					                    <a href="${pageContext.request.contextPath}/shop/content/movie/details/${categoryId}/${movie.id}">
					                    <img src="${!empty movie.defaultIcon ? movie.defaultIcon : movie.defaultImage}" alt="${movie.titleEN}"></a>
					                </div>
					                <div class="caption movieName">
					                <a href="${pageContext.request.contextPath}/shop/content/movie/details/${categoryId}/${movie.id}">
								      <c:choose>
								        <c:when test="${pageContext.response.locale.language eq 'en'}"><h6>${utils:abbreviate(movie.titleEN, 25)}</h6></c:when>
								        <c:otherwise><h6>${utils:abbreviate(movie.titleVI, 25)}</h6></c:otherwise>
								      </c:choose>										
					                </a></div>
					            </div>
					        </div>
