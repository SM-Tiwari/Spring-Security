			...
			<div id="appsBanner" class="carousel slide" data-ride="carousel" data-interval="3000">
				...
				<div class="carousel-inner">
					...
					<div class="item <c:if test="${ctr.index eq 0}">active</c:if>">
						<img src="${banner.defaultBanner}" alt="Apps Banner ${ctr.index}" style="width: 100%;"/>
					</div>
					</c:forEach>
				</div>
		...
		<div class="container">
			<div class="col-sm-10 col-sm-offset-1">
					...
					<div class="col-xs-6 sectionTitle-prev">
						...
						<a href="${pageContext.request.contextPath}/shop/content/apps/more/${categoryId}/${paging.number - 1}"><i class="fa fa-angle-left" aria-hidden="true"></i> <spring:message code="label.Back" text="Back"/></a>
						<%}%>								
					</div>
					<div class="col-xs-6 sectionTitle-next">
						<%if (paging.hasNext()){%>
						<a href="${pageContext.request.contextPath}/shop/content/apps/more/${categoryId}/${paging.number + 1}"><spring:message code="label.Next" text="Next"/> <i class="fa fa-angle-right" aria-hidden="true"></i></a>								
						<%}%>
					</div>
				</div>
				<!-- Section Movie List -->
				<div class="row marketPage-sectionGame">
					<c:forEach items="${appsList}" var="apps" varStatus="ctr">
						<div class="col-xs-6 col-sm-3">
				            <div class="thumbnail">
				                <div class="card-wrapper">
				                    <a href="${pageContext.request.contextPath}/shop/content/apps/details/${categoryId}/${apps.id}"><img src="${!empty apps.defaultIcon ? apps.defaultIcon : apps.defaultImage}" alt="${apps.titleEN}"></a>
				                </div>
				                <div class="caption gameName">
				                <a href="${pageContext.request.contextPath}/shop/content/apps/details/${categoryId}/${apps.id}">
								    <c:choose>
								      <c:when test="${pageContext.response.locale.language eq 'en'}"><h6>${utils:abbreviate(apps.titleEN, 25)}</h6></c:when>
								      <c:otherwise><h6>${utils:abbreviate(apps.titleVI, 25)}</h6></c:otherwise>
								    </c:choose>										
				                </a></div>
				            </div>
				        </div>								
